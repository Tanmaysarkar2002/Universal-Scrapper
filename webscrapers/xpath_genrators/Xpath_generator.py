                        ...
                        if element.has_attr(attr):
                            ...
                            if len(driver.find_elements(By.XPATH ,locator))==1:
                                ...
                                if  variable_name != '' and variable_name not in self.variable_names:
                                    ...
                                    self.a.append("%s_%s = %s"%(guessable_element, variable_name.encode('utf-8').decode('latin-1'), locator.encode('utf-8').decode('latin-1')))
                                    break
                                else:
                            ...
                            if len(driver.find_elements(By.XPATH ,locator))==1:
                                ...
                                if button_text.lower() not in self.button_text_lists:
                                    ...
                                    if not matches:
                                        ...
                                        self.a.append("%s_%s = %s"%(guessable_element,button_text.strip().strip("!?.").encode('utf-8').decode('latin-1').lower().replace(" + ","_").replace(" & ","_").replace(" ","_"), locator.encode('utf-8').decode('latin-1')))
                                    else:
                                        # printing the variable name with utf-8 characters along with language counter
                                        self.a.append("%s_%s_%s = %s"%(guessable_element,"foreign_language",self.language_counter, locator.encode('utf-8').decode('latin-1')) + "---> Foreign language found, please change the variable name appropriately")
                                        self.language_counter +=1
                                else:
...
def input_tag_genrator(url):
    #Initialize the xpath object
    global xpath_obj,driver
    ...
    chrome_options.binary_location = r"C:\Users\Tanmay\Downloads\chrome-win64\chrome-win64\chrome.exe"
    ...
    res = xpath_obj.generate_xpath(soup)
    ...
    if res[0] is False:
        ...
        print ("XPaths generated for the URL:%s"%url)
        # print (res)
        return True , res[1]
...
input_tag_genrator('https://www.rjiterp.org/rjit/index.php')
